...
class Window1(QWidget):
    ...
    def on_timer_timeout(self):
        """定时器超时后关闭窗口"""
        print("10 seconds passed, closing the animation window.")
        self.close()  # 关闭当前动画窗口
        self.window2 = CameraApp()  # 创建主窗口
        self.window2.show()  # 显示主窗口
    ...
    def update_exposure_time(self):
        ...
        try:
            ...
            if exposure_time <= 45 and exposure_time >= 0.02:  # 设置曝光时间的有效范围
                self.cam.ExposureTime = exposure_time * 1000  # 转换为微秒
        except ValueError:
            pass  # 如果输入无效，不做任何处理
